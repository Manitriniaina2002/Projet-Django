...
def Formateur_view(request):
    resultat_list = Formateur.objects.all()
    paginator = Paginator(resultat_list, 4)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'pages/Formateur.html', {'page_obj': page_obj})
...
def Supp_Formateur(request,id_formateur):
    if request.method == 'POST':
        ...
        FormationData.delete()
        messages.success(request,"Suppression effectue avec success !")
        return redirect('Formateur')
    else:
       return render(request,'pages/Formateur.html')
